...
Array.prototype.count_grand_children = function () {
    var sum = 0;
    for (var i = 0; i < this.length; i++) {
        // for (var j = 0; j < this[i].length; j++) {
        sum += this[i]['prefixes'].length;
        // console.log('Adding ' + this[i][j].length);
        // }
    }
    return sum;
};
...
function get_host_info(_host, res) {
    ...
    p_address.then(function (data) {
        ...
        res.send(500, 'API Error p_address.then');
    });
    p_asns.then(function (data) {
        var t_asnsv4 = data.v4_asns;
        var t_asnsv6 = data.v6_asns;
        var _prefixesv4 = [];
        var _prefixesv6 = [];
        ...
        res.send(500, 'API Error p_asns.then');
    });
    ...
    p_alldone.then(function (data) {
        ...
        res.send(500, 'API Error p_networks.then');
    });
}
